    ...
    <div class="footer-cta">
      <div class="container footer-cta-inner">
        ...
        <div class="cta-row">
          ...
          ${button({ label: "Join The Family", href: "/membership.html", variant: "accent" })}
        </div>
      </div>
    </div>
...
export function ministryCard(m, variant) {
  ...
  const img = m.image
    ...
    : "";
  return `
  <article class="ministry-card ministry-card-${style}${m.image ? " has-media" : ""}" id="${m.slug}">
    ${img}
...
export function eventCard(e) {
  ...
  <article class="event-card">
    <div class="event-card-media">
      <img src="${e.image}" alt="${esc(e.title)} — event flyer" loading="lazy" width="720" height="900">
      ${e.badge ? `<span class="event-card-badge">${esc(e.badge)}</span>` : ""}
    </div>
    <div class="event-card-body">
      <h3>${esc(e.title)}</h3>
      <p class="event-card-meta">${icon("calendar", "icon-xs")}<span>${esc(e.dateLabel)}${e.timeLabel ? ` · ${esc(e.timeLabel)}` : ""}</span></p>
      <p class="event-card-meta">${icon("map-pin", "icon-xs")}<span>${esc(e.location)}</span></p>
      <p>${esc(e.excerpt)}</p>
      ${
        e.cta
          ? button({ label: e.cta.label, href: e.cta.href, variant: "outline", icon: "arrow-right", external: !!e.cta.external })
          : ""
      }
    </div>
  </article>`;
